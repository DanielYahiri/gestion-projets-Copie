...
function Financier({ projetId }) {
  ...
  function badgePaiement(statut) {
    const config = {
      payee:      { label: 'Payé',       bg: 'var(--df-success-soft)', color: 'var(--df-success)' },
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="stagger-children" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        {[
          { label: 'Heures totales',     value: `${facturation.total_heures_travaillees}h`,                                              color: 'var(--df-text-primary)' },
          { label: 'Montant temps passé', value: `${Number(facturation.total_montant_temps_passe).toLocaleString('fr-FR')} FCFA`,        color: 'var(--df-accent)'       },
      ...
      <div className="df-card" style={{ padding: '24px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {[
            ...
            { label: 'Date de facturation',  value: facturation.date_facturation || '—' },
            { label: 'Notes de facturation', value: facturation.notes_facturation || '—' },
            { label: 'Membres actifs',       value: `${facturation.nb_membres_actifs_saved_time} membre(s)` },
          ].map(item => (
            <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--df-border)' }}>
              ...
              <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--df-text-primary)' }}>{item.value}</span>
            </div>
          ))}
        </div>
      </div>
